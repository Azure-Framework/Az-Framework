...
local resetHealthSnapshotRequested = false
local lastTeleportGuardCoords = nil
local lastTeleportGuardHealth = nil
local lastTeleportGuardArmor = nil
local lastTeleportGuardAt = 0

local function isLobbyTransitionActive()
  if not LocalPlayer or not LocalPlayer.state then return false end
  return LocalPlayer.state.az_lobby_selecting == true or LocalPlayer.state.az_lobby_teleporting == true
end

local function clearPedDamageMemory(ped)
  ped = ped or PlayerPedId()
  if ped == 0 or not DoesEntityExist(ped) then return end
  pcall(function() ClearEntityLastDamageEntity(ped) end)
  pcall(function() ClearPedLastWeaponDamage(ped) end)
end

local function markInjurySuppressed(msToSuppress, reason)
  msToSuppress = math.max(0, math.floor(tonumber(msToSuppress) or 0))
  if msToSuppress <= 0 then return end

  local untilAt = GetGameTimer() + msToSuppress
  if untilAt > injurySuppressUntil then
    injurySuppressUntil = untilAt
  end

  resetHealthSnapshotRequested = true
  recentTraumaContext = nil

  local ped = PlayerPedId()
  if ped ~= 0 and DoesEntityExist(ped) then
    clearPedDamageMemory(ped)
    local hp = GetEntityHealth(ped)
    local armor = GetPedArmour(ped)
    if hp and hp > 101 then lastTeleportGuardHealth = hp end
    if armor then lastTeleportGuardArmor = armor end
  end

  dprint(('Injury tracking suppressed for %sms%s'):format(msToSuppress, reason and (' [' .. tostring(reason) .. ']') or ''))
end

local function injuryTrackingBlocked()
  if injuryPauseState == true then return true end
  if isLobbyTransitionActive() then return true end
...
local function detectTeleportJumpNow(ped)
  local guard = Config.TeleportGuard or {}
  ...
  if not lastTeleportGuardCoords then
    lastTeleportGuardCoords = coords
    lastTeleportGuardHealth = GetEntityHealth(ped)
    lastTeleportGuardArmor = GetPedArmour(ped)
    lastTeleportGuardAt = GetGameTimer()
    return false
  end

  local dist = #(coords - lastTeleportGuardCoords)
  local dz = math.abs((coords.z or 0.0) - (lastTeleportGuardCoords.z or 0.0))
  local distanceLimit = tonumber(guard.Distance) or 45.0
  local verticalLimit = tonumber(guard.VerticalDistance) or 20.0

  lastTeleportGuardCoords = coords
  lastTeleportGuardAt = GetGameTimer()

  if dist >= distanceLimit or dz >= verticalLimit then
    markInjurySuppressed(tonumber(guard.SuppressMs) or 8000, ('teleport jump %.1fm / %.1fm z'):format(dist, dz))
    return true
  end

  if not injuryTrackingBlocked() and not isDowned then
    local hp = GetEntityHealth(ped)
    local armor = GetPedArmour(ped)
    if hp and hp > 101 then lastTeleportGuardHealth = hp end
    if armor then lastTeleportGuardArmor = armor end
...
local function restoreTeleportHealthIfNeeded(ped)
  local guard = Config.TeleportGuard or {}
  if guard.RestoreHealthDuringSuppression == false then return end
  if not injuryTrackingBlocked() then return end
  ped = ped or PlayerPedId()
  if ped == 0 or not DoesEntityExist(ped) or isDowned then return end

  local hp = GetEntityHealth(ped)
  local maxHp = GetEntityMaxHealth(ped)
  local restoreHp = tonumber(lastTeleportGuardHealth or 0) or 0
  if restoreHp > 101 and hp > 0 and hp < restoreHp then
    SetEntityHealth(ped, math.min(maxHp, restoreHp))
  end

  local armor = GetPedArmour(ped)
  local restoreArmor = tonumber(lastTeleportGuardArmor or 0) or 0
  if restoreArmor > 0 and armor < restoreArmor then
    SetPedArmour(ped, math.min(100, restoreArmor))
  end
end
...
AddEventHandler('baseevents:onPlayerDied', function()
  if injuryTrackingBlocked() or detectTeleportJumpNow(PlayerPedId()) then return end
...
AddEventHandler("gameEventTriggered", function(eventName, args)
  ...
  if injuryTrackingBlocked() or detectTeleportJumpNow(ped) then return end
...
CreateThread(function()
  ...
  while Config.VehicleImpact and Config.VehicleImpact.Enabled do
    ...
    if resetHealthSnapshotRequested then
      lastHp, lastArmor = hp, armor
      resetHealthSnapshotRequested = false
    end

    if injuryTrackingBlocked() or detectTeleportJumpNow(ped) then
      restoreTeleportHealthIfNeeded(ped)
...
CreateThread(function()
  while true do
    ...
    elseif IsEntityDead(ped) or isDowned then
      Wait(300)
    elseif injuryTrackingBlocked() or detectTeleportJumpNow(ped) then
      restoreTeleportHealthIfNeeded(ped)
      wasAirborne = false
...
CreateThread(function()
  local guard = Config.TeleportGuard or {}
  if guard.Enabled == false then return end

  local checkMs = tonumber(guard.CheckMs) or 250
  while true do
    Wait(checkMs)

    local ped = PlayerPedId()
    if ped == 0 or not DoesEntityExist(ped) then
      lastTeleportGuardCoords = nil
    else
      if isLobbyTransitionActive() then
        markInjurySuppressed(tonumber(guard.SuppressMs) or 8000, 'lobby transition state')
      end

      if detectTeleportJumpNow(ped) then
        wasAirborne = false
        airbornePeakHeight = 0.0
        airbornePeakSpeed = 0.0
      end
...
CreateThread(function()
  while true do
    ...
    if not isDowned then
      ...
      if injuryTrackingBlocked() or detectTeleportJumpNow(ped) then
        restoreTeleportHealthIfNeeded(ped)
      elseif (Config.Downed and Config.Downed.Enabled ~= false) and (IsEntityDead(ped) or IsPedFatallyInjured(ped) or IsPedDeadOrDying(ped, true) or GetEntityHealth(ped) <= math.max(101, tonumber((Config.Downed and Config.Downed.HealthOnDown) or 110) or 110)) then
...
RegisterNetEvent('Az-Death:injury:suppress', function(ms, reason)
  markInjurySuppressed(ms, reason or 'external suppress event')
end)

RegisterNetEvent('Az-Death:injury:teleporting', function(state, ms, reason)
  if state == false then return end
  markInjurySuppressed(ms or (Config.TeleportGuard and Config.TeleportGuard.SuppressMs) or 8000, reason or 'external teleport event')
end)

RegisterNetEvent('az-lobby:teleportStarting', function(ms)
  markInjurySuppressed(ms or (Config.TeleportGuard and Config.TeleportGuard.SuppressMs) or 8000, 'Az-LobbyRouter teleport')
end)

RegisterNetEvent('az-lobby:teleportFinished', function(ms)
  markInjurySuppressed(ms or 2500, 'Az-LobbyRouter teleport finish')
end)
...
exports('suppressInjuries', function(ms, reason)
  markInjurySuppressed(ms, reason or 'export suppress')
end)

exports('markTeleport', function(ms, reason)
  markInjurySuppressed(ms or (Config.TeleportGuard and Config.TeleportGuard.SuppressMs) or 8000, reason or 'export teleport')
end)
