...
function GetMoney(discordID, charID, callback)
    ...
    WHERE discordid = @discordid AND charid = @charid
      ...
      LIMIT 1
                ...
                function(fullName)
                    local first, last = fullName and fullName:match("^(%S+)%s+(%S+)$") or nil
                    first = first or ""
                    last = last or ""
            ...
            LIMIT 1
                    ...
                    if checkingBalance == nil then
                        exports.oxmysql:insert(
                            string.format(
                                "INSERT INTO `%s` (discordid,charid,type,balance) VALUES (?,?,?,?),(?,?,?,?)",
                                M.accts
                            ),
...
function depositMoney(source, amount)
    ...
    withMoney(
        ...
        function(dID, cID, data)
            ...
            exports.oxmysql:query(
                string.format(
                    "SELECT id,type,balance FROM `%s` WHERE (charid = ? OR discordid = ?) ORDER BY (charid = ?) DESC",
                    M.accts
                ),
                {cID, dID, cID},
                function(accts)
                        ...
                        exports.oxmysql:insert(
                            ...
                            function()
                                exports.oxmysql:query(
                                    ...
                                    function(accts2)
                                        local checking = nil
                                        for _, a in ipairs(accts2 or {}) do
                                        ...
                                        if not checking then
                                            return TriggerClientEvent(
                                                "chat:addMessage",
                                                source,
                                                {args = {"^1SYSTEM", "No checking account found (internal error)."}}
                                            )
                                        end
                                        local newBalance = (tonumber(checking.balance) or 0) + amount
                                        exports.oxmysql:execute(
                                            string.format(
                                                "UPDATE `%s` SET balance = ?, discordid = ?, charid = ? WHERE id = ?",
                                                M.accts
                                            ),
                                ...
                                UpdateMoney(
                                    ...
                                    function()
                                        ...
                                        TriggerClientEvent(
                                            "chat:addMessage",
                                            source,
                                            {args = {"^2SYSTEM", "Deposited $" .. amount .. " to checking."}}
                                        )
                                        if amount >= 1e6 then
                            ...
                            function(accts2)
                                ...
                                if current < amount then
                                    return TriggerClientEvent(
                                        "chat:addMessage",
                                        source,
                                        {args = {"^1SYSTEM", "Not enough bank funds to withdraw."}}
                                    )
                                end
                                ...
                                UpdateMoney(
                                    ...
                                    function()
                                        ...
                                        TriggerClientEvent(
                                            "chat:addMessage",
                                            source,
                                            {args = {"^2SYSTEM", "Withdrew $" .. amount .. " from checking."}}
                                        )
                                        if amount >= 1e6 then
                                            logLargeTransaction(
                                                "withdrawMoney",
...
function transferMoney(source, target, amount)
    ...
    if senderID == "" or not senderChar or targetID == "" or not targetChar then
        return TriggerClientEvent(
            "chat:addMessage",
            source,
            {args = {"^1SYSTEM", "Discord ID or character missing for sender/target."}}
        )
    end
        ...
        function(sData)
            if sData.cash < amount then
                return TriggerClientEvent(
                    "chat:addMessage",
                    source,
                    {args = {"^1SYSTEM", "Not enough cash to transfer."}}
                )
            end
            GetMoney(
                targetID,
                    ...
                    UpdateMoney(
                        ...
                        function()
                            ...
                            TriggerClientEvent(
                                "chat:addMessage",
                                target,
                                {args = {"^2SYSTEM", "You received $" .. amount}}
                            )
                            if amount >= 1e6 then
        ...
        function(dID, cID, data)
            ...
            if now - tonumber(data.last_daily) < 86400 then
                return TriggerClientEvent(
                    "chat:addMessage",
                    source,
                    {args = {"^1SYSTEM", "Daily reward already claimed."}}
                )
            end
            ...
            UpdateMoney(
                ...
                function()
                    ...
                    TriggerClientEvent(
                        "chat:addMessage",
                        source,
                        {args = {"^2SYSTEM", "Daily reward: $" .. rewardAmount}}
                    )
    ...
    function(source, args)
        ...
        if not targetId or not amount then
            return TriggerClientEvent(
                "chat:addMessage",
                source,
                {args = {"^1SYSTEM", "Usage: /transfer [id] [amount]"}}
            )
        end
...
AddEventHandler(
    ...
    function(firstName, lastName)
        ...
        debugPrint(
            ("registerCharacter: creating char %s for discord %s (player %d) name='%s'"):format(
                charID,
                discordID,
                src,
                fullName
        ...
        VALUES (@discordid, @charid, @name, '')
            ...
            function(affected)
                if not affected or affected < 1 then
                    debugPrint(
                        ("registerCharacter: failed to INSERT user_characters for %s / %s"):format(discordID, charID)
                    )
                    TriggerClientEvent(
                        "chat:addMessage",
                        src,
            ...
            VALUES
              (@discordid, @charid, @firstname, @lastname, 0, 0, 0, 'active')
                    ...
                    function(aff2)
                        if not aff2 or aff2 < 1 then
                            debugPrint(
                                ("registerCharacter: failed to INSERT econ_user_money for %s / %s"):format(
                                    discordID,
                                    charID
                                )
                            )
                            TriggerClientEvent(
                                "chat:addMessage",
                                src,
                                {args = {"^1SYSTEM", "Failed to initialize character economy. Check server logs."}}
                            )
                            return
                        end

                        debugPrint(
                            ("registerCharacter: inserted econ_user_money for %s / %s"):format(discordID, charID)
                        )
                        ...
                        if Config and Config.UseImperial == true then
                            ...
                            local ok, err =
                                ...
                                function()
                                    if
                                        exports["ImperialCAD"] and
                                            type(exports["ImperialCAD"].NewCharacter) == "function"
                                     then
                                        exports["ImperialCAD"]:NewCharacter(
                                            ...
                                            function(success, res)
                                                if success then
                                                    debugPrint(
                                                        ("ImperialCAD: NewCharacter succeeded for %s -> %s"):format(
                                                            charID,
                                                            json.encode(res)
                                                        )
                                                    )
                                                    ...
                                                    debugPrint(
                                                        ("ImperialCAD: NewCharacter failed for %s -> %s"):format(
                                                            charID,
                                                            json.encode(res)
                                                        )
                                                    )
                                        ...
                                        debugPrint(
                                            "ImperialCAD export not found or resource unavailable; skipping CAD creation."
                                        )
                                    end
                                end
                            )
                            ...
                            if not ok then
                                debugPrint(
                                    "Error while attempting to call ImperialCAD NewCharacter export: " ..
...
local function fetchMoney(did, cid, cb)
    exports.oxmysql:query(
        ...
        function(res)
            ...
            exports.oxmysql:insert(
                string.format(
                    "INSERT INTO `%s` (discordid,charid,cash,bank,profile_picture) VALUES (?,?,?,?, '')",
                    M.money
                ),
                {did, cid, 0, 0},
                function()
                    exports.oxmysql:query(
                        ...
                        {did, cid},
                        function(res2)
                            cb(res2 and res2[1] or {cash = 0, bank = 0})
                        end
                    )
                end
            )
...
Citizen.CreateThread(
    function()
        print(
            "Hourly-paycheck thread starting. Interval = " ..
                tostring(Config.PaycheckIntervalMinutes or 60) .. " minutes."
        )
                        ...
                        SELECT paycheck 
                         ...
                         LIMIT 1
                                ...
                                MySQL.Async.fetchScalar(
                                    ...
                                    function(paycheck)
                                        print(
                                            ("  ↳ lookup dept='%s' ids=[%s] → %s"):format(
                                                dept,
                                                table.concat(lookupIds, ","),
                                                tostring(paycheck)
                                            )
                                        )
